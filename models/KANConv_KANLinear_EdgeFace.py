...
from layers.Edge_layers.sdta_encoder import SDTAEncoder_KANConv_KANLinear
from layers.Edge_layers.conv_encoder import ConvEncoder_KANConv_KANLinear
from layers.KAN_layers.KANConv import KAN_Convolutional_Layer
from layers.Edge_layers.LoRaLin import LoRaLin_KAN



class EdgeFace_KANConv_KANLinear(nn.Module):
    def __init__(self, in_chans=3, num_classes=512,
        ...
        stem = nn.Sequential(
            KAN_Convolutional_Layer(in_chans, dims[0], kernel_size=4, stride=4),
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                ...
                KAN_Convolutional_Layer(dims[i], dims[i + 1], kernel_size=2, stride=2),
            )
            ...
            for j in range(depths[i]):
                if j > depths[i] - global_block[i] - 1:
                    if global_block_type[i] == 'SDTA':
                        stage_blocks.append(SDTAEncoder_KANConv_KANLinear(dim=dims[i], drop_path=dp_rates[cur + j],
                                                        expan_ratio=expan_ratio, scales=d2_scales[i],
                                                        use_pos_emb=use_pos_embd_xca[i], num_heads=heads[i]))
                    else:
                        raise NotImplementedError
                else:
                    stage_blocks.append(ConvEncoder_KANConv_KANLinear(dim=dims[i], drop_path=dp_rates[cur + j],
    ...
    def _init_weights(self, m):  # TODO: MobileViT is using 'kaiming_normal' for initializing conv layers
        if isinstance(m, (KAN_Convolutional_Layer, LoRaLin_KAN)):
